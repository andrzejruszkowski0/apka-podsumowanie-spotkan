...
- **Konto Google:** Google Workspace (własna domena).
...
4. Aplikacja w trybie **Testing** ma refresh token ważny 7 dni. **Trzeba przełączyć na Production** (Internal, skoro Workspace) — wtedy token nie wygasa.

**Ważne:** typ aplikacji w Google Cloud Console ustawić na **Internal** (możliwe dzięki Workspace). Pomija to weryfikację OAuth przez Google, która przy scope `gmail.send` byłaby wymagana dla aplikacji External.
...
POST   /briefing                    → {topic_id} → generuje i wysyła briefing
POST   /meetings/{id}/draft         → {template: formal_board|supplier|team_casual}
                                      → zwraca szkic; wysyłka osobnym potwierdzeniem
```
...
/meetings/:id      Stan przetwarzania (polling) → transkrypt
...
**Etap 2 — OAuth**
Google Cloud Console: projekt, OAuth client (Internal), scopes.
...
| Gmail API, Sheets API | 0 zł (w ramach limitów Workspace) |
...
| Refresh token wygasa | Utrata dostępu | Tryb Internal (nie Testing), obsługa 401 → ponowna zgoda |
